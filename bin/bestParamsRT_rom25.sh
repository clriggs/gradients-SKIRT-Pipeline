...
for i in "${!rom_dict[@]}"; do
    ...
    for j in "${!rom_dict[$i]}"; do
        echo "on halo: $j"
        python bestParamsRT.py --SF=True --tauClear=2.5 --sim=$i --sim_dict_path=$rom_path --halo=$j --distance=50 --numPhotons=1e8 --pixels=256 --dustFraction=0.1 --maxTemp=16000 --FoV=74240
    done
done